...
export const HistoryBar = () => { 
  ...
  const labels = ['Outubro', 'Novembro', 'Dezembro', 'Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro'];
  const data = {
    labels,
    datasets: [
      {
  ...
  return (
    <div className='bg-white shadow-md rounded-xl w-full px-6 py-6 flex flex-col'>
      <h3 className='font-semibold text-2xl text-emerald-600 pb-6'>Histórico de cobranças</h3>
      <Bar options={options} data={data} width={1600} height={300} style={{width: '100%'}}/>
    </div>
  )
}